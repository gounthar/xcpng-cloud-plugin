...
**Test 2 (read-back integrity): the import is BYTE-PERFECT, not truncated.** `VDI.create` (3 GiB), then
`import_raw_vdi` PUT (my exact `urllib`, file-object body), then `SR.scan`, then `export_raw_vdi`, then sha512.
...
- `xe vm-install template="Debian Bookworm 12"`, then destroy the auto-provisioned disk, then attach the
  byte-perfect Bookworm import (device 0 bootable), then a VIF, then `vm-start`. **Same failure**: 4.9 CPU-s,
...
**Reproduced today**: golden's exact recipe was `deb.raw` (dev 0) + `cidata.iso` seed (dev 1) +
`guest-tools.iso` CD (dev 3) + 2 vCPU, via `xe vm-install "Debian Bookworm 12"`. It **booted**,
`0/ip 192.168.1.139` at ~90s, CPU-s climbing 5.9 to 21. Drop the seed and CD and the identical raw
...
(dev 3) + 2 vCPU + `cdn`, and it **booted**, `0/ip 192.168.1.152`. So the recipe is dependable: 4/4 across
...
template default, which stalled), then set 2 vCPU and restarted, and it still stalled. A *clean* 2-vCPU first boot
...
(`-vnc unix:/var/run/xen/vnc-<domid>`, then `vncdo capture`). The screen showed:
...
**The VM booted.** OVMF handed off to grub, then the kernel, then systemd, then a getty login prompt on tty1. There is no
...
| no guest-reported IP | stalled | no network, so the guest agent has nothing to report |
| - | - | hostname is `localhost`, so **cloud-init never applied its per-instance config** |
...
network, so no DHCP, so no IP, which looks, through our proxies, exactly like a boot stall. golden's *clones*
...
stack that is: `xe vm-list params=dom-id`, then on dom0 `-vnc unix:/var/run/xen/vnc-<domid>`, then SSH-forward
the unix socket, then `vncdo capture`. Step one, not step fifty.
...
from the `jenkins-golden-debian` template (proven UEFI shell), then stripped golden's cloned disk, then attached
a fresh `import_raw_vdi` of dom0's `/var/tmp/deb.raw` (dev0) + a `genisoimage` NoCloud seed (user-data +
meta-data with a unique `instance-id` + the repo's `network-config`) as dev1, then `vm-start`. After ~80 s:
